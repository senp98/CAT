...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="text2image-fine-tune-dreambooth",
        help=(
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help=("Whether to use the VAE adapter model for cat."),
    )
    parser.add_argument(
        "--cat_adapter_path",
        type=str,
        default=None,
        help=("The path to the VAE adapter model used for cat."),
    )
    parser.add_argument(
        "--rank",
        type=int,
    ...
    parser.add_argument(
        ...
        default="encoder",
        help=(
            "Add LoRA to which module of the VAE. Choose between ['both', 'encoder', 'decoder']."
        ),
...
def main(args):
    ...
    if model_has_vae(args):
        ...
        if args.cat_adapter:
            ...
            peft_state_dict = load_file(args.cat_adapter_path)
            peft_state_dict = {k.replace("vae.", ""): v for k, v in peft_state_dict.items() if k.startswith("vae.")}
            peft_state_dict = {
                k.replace("lora.up", "lora_B") if "lora.up" in k else k: v for k, v in peft_state_dict.items()
            }
            peft_state_dict = {
                k.replace("lora.down", "lora_A") if "lora.down" in k else k: v for k, v in peft_state_dict.items()
            }

            outcome = set_peft_model_state_dict(vae, peft_state_dict)
            assert outcome.unexpected_keys == [], f"Unexpected keys: {outcome.unexpected_keys}"
            print(f"Successfully loaded the CAT adapter from {args.cat_adapter_path}")
    else:
        vae = None
